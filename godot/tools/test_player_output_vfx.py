...
def main() -> None:
    ...
    require("OUTPUT_FLASH_TIME := 0.13" in player_source and "OUTPUT_PARTICLE_COUNT := 18" in player_source, "Hot flash must follow the duckflock reference spec (1-2 frames); particles/trail carry readability during rapid spawning")
    ...
    require("best_score_label.custom_minimum_size = Vector2(0.0, 20.0)" in title_source, "Best score label needs its own safe spacing band")
